...
def parse_log(logname):
    ...
    # Detect already-normalized data structurally: a base PSD CSV has the
    # five columns freq,psd_x,psd_y,psd_z,psd_xyz; any extra column is a
    # shaper response written by save_calibration_data, which means the
    # PSDs were already normalized to input frequencies before saving.
    if len(header.rstrip().split(',')) <= 5:
