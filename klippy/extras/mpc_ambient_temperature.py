# Virtual sensor that exposes an MPC heater's internal ambient state
#
...
class MpcAmbientTemperature:
    ...
    def process_temp_update(self, control, read_time):
        ...
        if ctype == 'mpc':
            self.temp = control.state_ambient_temp
        else:
            # During MPC_CALIBRATE the active control is TuningControl which
            # has no state_ambient_temp; fall back to the fixed ambient
            # constant so the virtual sensor keeps reporting a plausible
...
def load_config(config):
    ...
    pheaters.add_sensor_factory('mpc_ambient_temperature',
                                MpcAmbientTemperature)
