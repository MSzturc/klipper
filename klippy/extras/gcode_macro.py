...
# Reloadable wrapper around TemplateWrapper. Existing references to the
# Template instance keep working after reload() swaps out the underlying
# template; attribute access (.render, .run_gcode_from_command) is forwarded
# to whichever TemplateWrapper is currently active.
class Template:
    def __init__(self, printer, env, name, script):
        self.name = name
        self.printer = printer
    ...
    def reload(self, script):
        self.function = TemplateWrapper(self.printer, self.env, self.name,
                                        script)
...
class PrinterGCodeMacro:
    def __init__(self, config):
        ...
        self.gcode.register_command(
            ...
            desc=self.cmd_RELOAD_GCODE_MACROS_help)
    ...
    cmd_RELOAD_GCODE_MACROS_help = ("Re-read printer.cfg and reload the "
                                    "gcode of every [gcode_macro] section "
                                    "without restarting Klipper")
    def cmd_RELOAD_GCODE_MACROS(self, gcmd):
        pconfig = configfile.PrinterConfig(self.printer)
        new_config = pconfig.read_main_config()
        new_macro_names = {s.get_name()
                           for s in new_config.get_prefix_sections(
                               "gcode_macro ")}
        for name, obj in self.printer.lookup_objects("gcode_macro"):
            if name not in new_macro_names:
                continue
            try:
            ...
            obj.template.reload(new_script)
