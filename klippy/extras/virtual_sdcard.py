...
class VirtualSDGCodeProvider:
    ...
    def cmd_M26(self, gcmd):
        ...
        if sdcard.is_active() and not sdcard.is_cmd_from_sd():
            gcmd.respond_raw("SD busy")
            return
        if not self.is_active():
            gcmd.respond_raw("Not printing from SD card.")
        pos = gcmd.get_int('S', minval=0)
        # Update both file_position (used by get_gcode's initial seek) and
        # next_file_position (consumed by the running generator's seek check).
...
class VirtualSD:
    ...
    def do_resume(self):
        ...
        if self.gcode_provider is None:
            raise self.gcode.error("No SD file loaded")
