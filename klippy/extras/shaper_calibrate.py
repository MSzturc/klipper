...
class ShaperCalibrate:
    ...
    def background_process_exec(self, method, args):
        ...
        calc_proc.start()
        # Wait for the result to arrive on the pipe. Polling parent_conn
        # (instead of calc_proc.is_alive()) drains the pipe as soon as
        # the child calls send(); otherwise a result larger than the pipe
        # buffer would block the child in send() while the parent waited
        # for the process to exit, causing a deadlock.
        reactor = self.printer.get_reactor()
        gcode = self.printer.lookup_object("gcode")
        eventtime = last_report_time = reactor.monotonic()
        while not parent_conn.poll():
            if not calc_proc.is_alive():
                raise self.error("Calculation process terminated unexpectedly")
