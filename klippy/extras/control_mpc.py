...
class ControlMPC:
    def __init__(self, profile, heater, load_clean=False, register=True):
        ...
        # Re-registering MPC_CALIBRATE / MPC_SET raises config_error on
        # duplicate mux keys (e.g. when load_profile switches to a new
        # ControlMPC instance).  Update the handler in place instead.
        for cmd, func, desc in (
            ("MPC_CALIBRATE", self.cmd_MPC_CALIBRATE,
             self.cmd_MPC_CALIBRATE_help),
            ("MPC_SET", self.cmd_MPC_SET, self.cmd_MPC_SET_help),
        ):
            prev = gcode.mux_commands.get(cmd)
            if prev is not None and heater.short_name in prev[1]:
                prev[1][heater.short_name] = func
            else:
                gcode.register_mux_command(
                    cmd, "HEATER", heater.short_name, func, desc=desc
                )
    ...
    def cmd_MPC_SET(self, gcmd):
        ...
        # Sync live changes back to the profile dict so profile switches and
        # SAVE_CONFIG can persist the runtime-tuned values.
        self.profile["filament_diameter"] = self.const_filament_diameter
        self.profile["filament_density"] = self.const_filament_density
        self.profile["filament_heat_capacity"] = (
            self.const_filament_heat_capacity)
        self.profile["block_heat_capacity"] = self.const_block_heat_capacity
        self.profile["sensor_responsiveness"] = (
            self.const_sensor_responsiveness)
        self.profile["ambient_transfer"] = self.const_ambient_transfer
        self.profile["fan_ambient_transfer"] = self.const_fan_ambient_transfer
        self.profile["filament_temp_src"] = self.filament_temp_src
        cfgname = self.heater.get_name()
        configfile = self.printer.lookup_object("configfile")
        if self.const_block_heat_capacity is not None:
            configfile.set(cfgname, "block_heat_capacity",
                           f"{self.const_block_heat_capacity:#.6g}")
        if self.const_sensor_responsiveness is not None:
            configfile.set(cfgname, "sensor_responsiveness",
                           f"{self.const_sensor_responsiveness:#.6g}")
        if self.const_ambient_transfer is not None:
            configfile.set(cfgname, "ambient_transfer",
                           f"{self.const_ambient_transfer:#.6g}")
        configfile.set(cfgname, "fan_ambient_transfer",
                       ", ".join(f"{v:#.6g}"
                                 for v in self.const_fan_ambient_transfer))
        configfile.set(cfgname, "filament_diameter",
                       f"{self.const_filament_diameter:#.6g}")
        configfile.set(cfgname, "filament_density",
                       f"{self.const_filament_density:#.6g}")
        configfile.set(cfgname, "filament_heat_capacity",
                       f"{self.const_filament_heat_capacity:#.6g}")
        # Persist filament_temperature_source so MPC_SET FILAMENT_TEMP=…
        # survives a printer restart.
        src = self.filament_temp_src
        if src[0] == FILAMENT_TEMP_SRC_FIXED:
            filament_temp_src_str = f"{src[1]:#.6g}"
        else:
            filament_temp_src_str = src[0]
        configfile.set(cfgname, "filament_temperature_source",
                       filament_temp_src_str)
