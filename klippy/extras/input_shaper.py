...
class TypedInputShaperParams:
    shapers = {s.name : s for s in shaper_defs.INPUT_SHAPERS}
    def __init__(self, axis, shaper_type, config):
        ...
        if config is not None:
            ...
            self.damping_ratio = config.getfloat(
                    'damping_ratio_' + axis, self.damping_ratio, minval=0.,
                    maxval=self.shapers[shaper_type].max_damping_ratio)
    ...
    def update(self, shaper_type, gcmd):
        ...
        if damping_ratio > self.shapers[shaper_type].max_damping_ratio:
            raise gcmd.error(
...
class AxisInputShaper:
    ...
    def update_extruder_kinematics(self, sk):
        ...
        if not self.is_enabled():
            ...
            C_e, t_sm = shaper_defs.get_none_smoother()
            ffi_lib.extruder_set_smoothing_params(
                    sk, axis, len(C_e), C_e, t_sm, 0.)
            return True
...
class TypedInputSmootherParams:
    smoothers = {s.name : s for s in shaper_defs.INPUT_SMOOTHERS}
    def __init__(self, axis, smoother_type, config):
        ...
        if config is not None:
            ...
            if self._supports_damping_ratio(smoother_type):
                self.damping_ratio = config.getfloat(
                        'damping_ratio_' + axis, self.damping_ratio, minval=0.,
                        maxval=self.smoothers[smoother_type].max_damping_ratio)
    ...
    def _supports_damping_ratio(self, smoother_type):
        getargspec = inspect.getfullargspec if sys.version_info.major >= 3 \
                else inspect.getargspec
        init_func = self.smoothers[smoother_type].init_func
        return 'damping_ratio' in getargspec(init_func).args
    def get_type(self):
    ...
    def update(self, smoother_type, gcmd):
        ...
        if self._supports_damping_ratio(smoother_type):
            damping_ratio = gcmd.get_float('DAMPING_RATIO_' + axis,
                                           self.damping_ratio, minval=0.)
            max_damping_ratio = \
                    self.smoothers[smoother_type].max_damping_ratio
            if damping_ratio > max_damping_ratio:
                raise gcmd.error(
                        'Too high value of damping_ratio=%.3f for smoother %s'
                        ' on axis %c' % (damping_ratio, smoother_type, axis))
            self.damping_ratio = damping_ratio
        else:
            self.damping_ratio = shaper_defs.DEFAULT_DAMPING_RATIO
    ...
    def get_smoother(self):
        if not self.smoother_freq:
            ...
            C, tsm = self.smoothers[self.smoother_type].init_func(
                    self.smoother_freq, self.damping_ratio,
                    normalize_coeffs=False)
        return len(C), C, tsm
    def get_status(self):
        return collections.OrderedDict([
...
class AxisInputSmoother:
    ...
    def update_extruder_kinematics(self, sk):
        ...
        C_e, t_sm = extruder_smoother.get_extruder_smoother(
                ...
                damping_ratio, normalize_coeffs=False)
...
class InputShaper:
    def __init__(self, config):
        ...
        self.shapers = [self.shaper_factory.create_shaper('x', config),
                        ...
                        self.shaper_factory.create_shaper('z', config)]
