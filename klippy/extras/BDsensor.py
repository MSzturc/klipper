...
# When pin_params['pin'] originates from a [board_pins] alias, lookup_pin()
# returns the alias verbatim instead of the underlying hardware pin. The
# resolved value must be sent to the MCU; consult the pin_resolver's
# aliases table to unwrap one level. Used at every site that consumes
# pin_params['pin'] for raw MCU configuration (sda, scl, endstop).
def _resolve_pin_alias(ppins, pin_params):
    pin_resolver = ppins.get_pin_resolver(pin_params['chip_name'])
    pin = pin_params['pin']
    return pin_resolver.aliases.get(pin, pin)
...
class BDPrinterProbe:
    ...
    def start_probe_session(self, gcmd):   
        ...
        if "BED_MESH_CALIBRATE" in gcmd.get_command():
            try:
                if self.mcu_probe.no_stop_probe:
    ...
    def _probe_external_endstop(self, speed):
        ...
        # Allow axis_twist_compensation to update results — listeners
        # mutate the list in place, so re-read epos from index 0 instead
        # of returning the pre-event reference.
        results = [self.probe_offsets.create_probe_result(ppos)]
    ...
    def _probe(self, speed):
        ...
        # Allow axis_twist_compensation to update results — re-read from
        # the list because listeners replace the element in place.
        results = [self.probe_offsets.create_probe_result(ppos)]
        self.printer.send_event("probe:update_results", results)
        epos = results[0]
        # Report results
    ...
    def _calc_mean(self, positions):
        ...
        means = [sum([pos[i] for pos in positions]) / count
                 for i in range(3)]
        return manual_probe.ProbeResult(means[0], means[1], means[2],
        ...
        while self._probe_times:
            ...
            while 1:
                ...
                if est_print_time>=pos_time:
                    ...
                    # Allow axis_twist_compensation to update results — the
                    # event listener replaces the list element in place, so
                    # epos is read back from the list before being recorded.
                    results = [self.probe_offsets.create_probe_result(pos)]
                    self.printer.send_event("probe:update_results", results)
                    epos = results[0]
        ...
        while len(positions) < sample_count:
            ...
            try:
                if ((self.mcu_probe is not None) and
                    ...
                    # re-read epos from the list after the event.
                    results = [self.probe_offsets.create_probe_result(pos)]
                    self.printer.send_event("probe:update_results", results)
                    epos = results[0]
                    # Report results
...
class BDsensorEndstopWrapper:
    def __init__(self, config):
        ...
        self.no_stop_probe = config.getboolean('no_stop_probe', False)
        ...
        mcu = pin_params['chip']
        self.sda_pin_num = _resolve_pin_alias(ppins, pin_params)
        self.mcu = mcu
        # print("b2:%s"%mcu)
        pin_s = []
        try:
        ...
        mcu = pin_params['chip']
        scl_pin_num = _resolve_pin_alias(ppins, pin_params)
        # print("b3:%s"%mcu)
        # pin_params['pullup']=2
        ...
        # endstop_pin is optional; only swallow the missing-option case so
        # a typo in the pin name surfaces as a startup error instead of
        # silently falling back to the SDA pin.
        pin = config.get('endstop_pin', None)
        if pin is not None:
            pin_params = ppins.lookup_pin(pin, can_invert=True, can_pullup=True)
            self.endstop_pin_num = _resolve_pin_alias(ppins, pin_params)
    ...
    def bd_set(self, gcmd):
        ...
        if cmd_bd is not None:
            self.no_stop_probe = bool(cmd_bd)
            configfile = self.printer.lookup_object('configfile')
            configfile.set(self.name, 'no_stop_probe', "%d" % cmd_bd)
            gcmd.respond_info("no_stop_probe is set to %d. The SAVE_CONFIG "
